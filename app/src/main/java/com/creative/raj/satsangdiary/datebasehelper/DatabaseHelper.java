...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        // creating tables
        db.execSQL("create table if not exists area(area_id integer PRIMARY KEY AUTOINCREMENT, area_name varchar(20) UNIQUE)");
        db.execSQL("create table if not exists center(center_id integer PRIMARY KEY AUTOINCREMENT, center_name varchar(25) UNIQUE)");
        ...
        db.execSQL("create table if not exists area_center_relation(area_id INTEGER REFERENCES area (area_id) ON DELETE CASCADE, center_id INTEGER REFERENCES center (center_id) ON DELETE CASCADE)");
